...
class SavingsSimulation:
    ...
    def save_to_file(self, out_file, force = False):
        if not force and os.path.isfile(out_file):
            raise Exception('File "{}" already exists'. format(out_file))
    ...
    @classmethod
    def load_from_file(cls, in_file):
        if not os.path.isfile(in_file):
            raise Exception('"{}" is not a file'.format(in_file))
        try:
        ...
        if not isinstance(_d, dict):
            raise Exception('Data in "{}" not readale'.format(in_file))
        _gs = cls(
            _d['current_age'], _d['retirement_age'], _d['expected_age'],
            _d['initial_invest'], _d['monthly_withdrawal_nominal'],
            _d['rate_of_return'], _d['rate_of_inflation'],
            _d['flat_tax_rate'], _d['solidarity_tax'], _d['church_tax'],
            _d['notes'])
        _gs._is_saved = True
        return _gs
